...
date = headers.pop(0)
num = headers.pop(0)
seller = str(headers.pop(0)).replace("\\n", "\n")
shipFrom = str(headers.pop(0)).replace("\\n", "\n")
payTo = str(headers.pop(0)).replace("\\n", "\n")
...
header = [["MEL-TEX", "Purchase Order", num + "\n\nDate: " + date]]
...
info = [["Seller:", "Ship From:", "Pay To:"],
        [seller, shipFrom, payTo]]
t1 = Table(info, colWidths=width * 0.33, rowHeights=80, spaceBefore=0)
